...
Create a CLi script run-sql.tsat the repo root taking the path to a .sql file, connects via pg using DATABASE_URL, ececutes it, and reports the result. Add it as run-sql script to the root package.json, invoking via node --env-file=__.